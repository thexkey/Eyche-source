...
local success, err = pcall(function()	
	...
	if not playerConnectSucces then
		...
		player = game:GetService("Players"):CreateLocalPlayer(<?php echo $id; ?>)
		analytics("Created Player")
		client:Connect("<?php echo $ip; ?>", <?php echo $port; ?>, 0, threadSleepTime)
	else
		analytics("Created Player")
	end
	...
	pcall(function() player.Name = [========[<?php echo $username; ?>]========] end)
